...
export default defineConfig([
  {
    ...
    external: [/^@deepseek-ai\//, /^node:/, 'ssh2'], sourcemap: true, dts: true,
  },
  {
    entry: { client: 'src/client/index.ts' }, outDir: 'lib', format: 'cjs', platform: 'browser',
    external: clientExternal, noExternal: (id: string) => clientExternal.includes(id) ? undefined : true,
    sourcemap: true, clean: false, dts: false,
    outputOptions: {
      entryFileNames: 'client.js',
      banner: 'window.__ModuleLoader__.load({ id: "dsh-withssh", factory: (require) => {',
